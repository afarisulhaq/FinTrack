...
export function parseFinanceText(text) {
    ...
    // ─── Category ──────────────────────────────────────────────────────
    const category = lower.includes("kopi")
        ? "Kopi & Minuman"
                ...
                : lower.includes("mandiri")
                    ...
                    : lower.includes("gopay") || lower.includes("ovo") || lower.includes("dana") || lower.includes("shopeepay")
                        ...
                        : lower.includes("kantong")
                            ? "Kantong Jajan"
                            : "Dompet Utama");
    const type = lower.includes("gaji") || lower.includes("masuk") ? "income" : "expense";
    const categoryIcon = category === "Makan"
        ? "🍽️"
 ...
 */
export async function ocrReceiptImage(imageBuffer) {
    try {
        ...
        const totalRe = /(?:total|jumlah|subtotal|grand\s*total|amount|bayar|dibayar|kembali)\s*[:=]?\s*(?:rp|Rp)?\s*([\d.,\s]+)/i;
