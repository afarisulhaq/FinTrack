...
export function setWhatsAppConnectedUser(userId) {
 ...
 */
function findBestWallet(walletName, wallets) {
...
export async function startWhatsAppBot() {
    ...
    try {
        ...
        activeSocket.ev.on("contacts.upsert", async (contacts) => {
            for (const contact of contacts) {
                if (contact?.id || contact?.jid) {
                    const cid = contact.id || contact.jid;
                    knownContacts.set(cid, contact.name ||
                        contact.notify ||
        ...
        activeSocket.ev.on("messages.upsert", async (event) => {
            for (const message of event.messages ?? []) {
                ...
                if (hasImage && (!text || parseFinanceText(text).amount <= 0)) {
                    try {
                        ...
                        if (ocrAmount) {
                            ...
                            console.log(`[WhatsApp] OCR extracted: ${ocrAmount}, combined: "${text}"`);
                        }
                    }
                    catch (err) {
                ...
                try {
                    ...
                    const errMsg = err instanceof Error ? err.message : "Gagal memproses pesan";
...
async function resolveUserFromPhone(phone) {
    ...
    try {
        ...
        if (phone) {
            ...
            if (strippedPhone.length >= 10) {
                ...
                if (altSetting?.userId)
                    return altSetting.userId;
            }
        }
        ...
        console.warn("[WhatsApp] resolveUserFromPhone DB error:", error instanceof Error ? error.message : error);
    }
    return null;
}
/**
 ...
 */
async function handleWASaldo(userId) {
 ...
 */
async function handleWALaporan(userId) {
    const now = new Date();
...
export async function startWhatsAppBotWithPairing(phone) {
    ...
    try {
        ...
        if (pairingPhone && !isRegistered && activeSocket.requestPairingCode) {
            try {
                ...
                whatsappState = {
                    ...whatsappState,
                    lastError: error instanceof Error ? error.message : "Pairing request failed",
                };
            }
        }
        activeSocket.ev.on("messages.upsert", async (event) => {
...
export async function sendWhatsAppMessage(phone, text) {
    ...
    try {
        ...
        return {
            ok: false,
            error: error instanceof Error ? error.message : "Pesan WhatsApp gagal dikirim",
        };
    }
}
/**
 ...
 */
function getBaileysErrorMessage(error) {
    if (!error)
        return "unknown";
...
function isSessionCorruptionError(error) {
    ...
    return (msg.includes("connection failure") ||
