...
const DIRECT_YAHOO_SEARCH_URL = "https://query2.finance.yahoo.com/v1/finance/search";
...
const yahooAuthHeaders = {};
if (YAHOO_COOKIE)
    yahooAuthHeaders["Cookie"] = YAHOO_COOKIE;
if (YAHOO_CRUMB) {
...
function tripYahooCooldown() {
    ...
    console.warn(`[market-price] Yahoo Finance rate-limited; cooling down for ${YAHOO_COOLDOWN_MS / 1000}s`);
}
...
const PERSISTENT_CACHE_PATH = process.env.YAHOO_PERSISTENT_CACHE_PATH ??
...
const persistentQuoteCache = new Map();
const persistentPriceCache = new Map();
...
function loadPersistentCache() {
    ...
    try {
        ...
        console.log(`[market-price] loaded ${persistentQuoteCache.size} quotes, ${persistentPriceCache.size} prices from persistent cache at ${PERSISTENT_CACHE_PATH}`);
    }
    catch (err) {
        console.warn("[market-price] failed to load persistent cache:", err instanceof Error ? err.message : err);
    }
}
function schedulePersistentWrite() {
    persistentCacheDirty = true;
    if (persistentWriteTimer)
        return;
    ...
    persistentWriteTimer = setTimeout(() => {
        ...
        try {
            ...
            fs.writeFileSync(PERSISTENT_CACHE_PATH, JSON.stringify(snapshot), "utf-8");
        }
        catch (err) {
            console.warn("[market-price] failed to write persistent cache:", err instanceof Error ? err.message : err);
        }
    }, 2000);
}
...
async function fetchYahooChart(symbol, range) {
    ...
    for (const host of YAHOO_CHART_HOSTS) {
        ...
        try {
            ...
            console.warn(`[market-price] fetch error from ${host} for ${symbol}:`, err instanceof Error ? err.message : err);
            continue;
        }
        if (response.ok)
    ...
    if (lastAttemptStatus.length > 0) {
        console.warn(`[market-price] all Yahoo hosts failed for ${symbol}; last statuses: ${lastAttemptStatus.join(",")}`);
    }
    return null;
}
export async function getYahooFinanceQuote(symbol, assetClass = "stock") {
    ...
    if (!attempt) {
        ...
        if (cached) {
            console.warn(`[market-price] quote fetch failed for ${cacheKey}; serving stale in-memory cache`);
            return cached.value;
        }
        const persisted = persistentQuoteCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] quote fetch failed for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
        return null;
    }
    ...
    if (response.status === 429) {
        ...
        console.warn(`[market-price] quote fetch 429 for ${cacheKey} from ${host}; no cache, returning null`);
        return null;
    }
    ...
    if (response.status === 401 || response.status === 403) {
        ...
        const persisted = persistentQuoteCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] quote fetch ${response.status} for ${cacheKey} from ${host}; serving persistent cache (set YAHOO_COOKIE + YAHOO_CRUMB to authenticate)`);
            return persisted.value;
        }
        return null;
    }
    ...
    if (!meta) {
        ...
        const persisted = persistentQuoteCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] quote response had no meta for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
    ...
    if (!price) {
        ...
        const persisted = persistentQuoteCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] quote response had no price for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
        writeCache(quoteCache, cacheKey, null, QUOTE_TTL_MS);
        return null;
...
export async function getYahooFinancePrice(symbol, assetClass = "stock") {
    ...
    if (!attempt) {
        if (cached) {
            console.warn(`[market-price] price fetch failed for ${cacheKey}; serving stale in-memory cache`);
            return cached.value;
        }
        const persisted = persistentPriceCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] price fetch failed for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
        return null;
    }
    ...
    if (response.status === 429) {
        ...
        if (persisted)
            return persisted.value;
        console.warn(`[market-price] price fetch 429 for ${cacheKey} from ${host}; no cache, returning null`);
        return null;
    }
    if (response.status === 401 || response.status === 403) {
        if (cached)
    ...
    if (!meta) {
        ...
        const persisted = persistentPriceCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] price response had no meta for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
    ...
    if (!price) {
        const persisted = persistentPriceCache.get(cacheKey);
        if (persisted) {
            console.warn(`[market-price] price response had no price for ${cacheKey}; serving persistent cache`);
            return persisted.value;
        }
        writeCache(priceCache, cacheKey, null, PRICE_TTL_MS);
        return null;
...
export async function prewarmCommonSymbols() {
    if (prewarmStarted)
        return;
    prewarmStarted = true;
    loadPersistentCache();
    if (inYahooCooldown()) {
        console.log("[market-price] prewarm skipped: Yahoo is in cooldown, using existing persistent cache");
        return;
    }
    if (IDX_DEFAULT_SYMBOLS.length === 0)
        return;
    console.log(`[market-price] prewarming ${IDX_DEFAULT_SYMBOLS.length} common symbols...`);
    let ok = 0;
    let failed = 0;
    await Promise.allSettled(IDX_DEFAULT_SYMBOLS.map(async ({ symbol, assetClass }) => {
        const result = await getYahooFinanceQuote(symbol, assetClass).catch(() => null);
        if (result)
            ok++;
        else
            failed++;
    }));
    console.log(`[market-price] prewarm complete: ${ok} ok, ${failed} failed (persistent cache now has ${persistentQuoteCache.size} quotes)`);
}
