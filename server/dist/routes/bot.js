...
import { handleTelegramUpdate, sendTelegramMessage, telegramStatus, verifyTelegramToken, getTelegramContacts, } from "../services/telegram.js";
import { resetWhatsAppSession, sendWhatsAppMessage, setWhatsAppConnectedUser, startWhatsAppBot, startWhatsAppBotWithPairing, stopWhatsAppBot, whatsappStatus, getWhatsAppContacts, } from "../services/whatsapp.js";
...
async function getUserTelegramConfig(userId) {
    ...
    try {
        ...
        if (!setting || !setting.channelTGBot)
            return null;
        return {
...
async function saveBotConnection(channel, status, enabled, meta) {
    if (!(await canUseDatabase()))
        return;
    try {
        await prisma.botConnection.upsert({
            where: { channel },
            update: { status, enabled, meta: meta },
            create: {
                channel,
                status,
                enabled,
        ...
        console.warn("BotConnection table unavailable, skipping bot status persistence:", error instanceof Error ? error.message : error);
    }
}
export const botRoutes = new Elysia({ prefix: "/api/bot" })
    ...
    if (await canUseDatabase()) {
        await Promise.all([
            saveBotConnection("whatsapp", whatsapp.status, whatsapp.status === "connected", {
                qrReady: Boolean(whatsapp.qrDataUrl),
                lastError: whatsapp.lastError,
                pairingCode: whatsapp.pairingCode,
            }),
            saveBotConnection("telegram", telegram.configured ? "configured" : "unconfigured", Boolean(telegram.configured), {
    ...
    const userId = getUserIdFromRequest(request);
    const userConfig = await getUserTelegramConfig(userId);
    const result = await sendTelegramMessage(data.text, userConfig?.chatId ?? process.env.TELEGRAM_DEFAULT_CHAT_ID, userConfig?.botToken);
    return ok(result);
})
    ...
    if (lower === "/saldo" && userId && (await canUseDatabase())) {
        try {
            ...
            if (wallets.length === 0)
                return ok({ reply: "Belum ada dompet." });
    ...
    if (lower === "/laporan" && userId && (await canUseDatabase())) {
        try {
            ...
            if (expenses.length === 0)
                return ok({ reply: "📊 Belum ada transaksi minggu ini." });
    ...
    if (parsed.amount > 0 && userId && (await canUseDatabase())) {
        try {
            ...
            const findWallet = (name) => {
                ...
                if (exact)
                    return exact;
                const noPrefix = wallets.find((w) => w.name.toLowerCase().replace(/^dompet\s+/i, "") === lower);
                if (noPrefix)
                    return noPrefix;
