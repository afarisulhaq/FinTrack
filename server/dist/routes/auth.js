...
async function verifyTurnstileToken(token) {
    ...
    if (!secret) {
        ...
        console.warn("[auth] TURNSTILE_SECRET_KEY not set; skipping Turnstile verification");
        return true;
    }
    if (!token)
        return false;
    try {
        ...
        if (!response.ok) {
            console.warn(`[auth] Turnstile siteverify HTTP ${response.status}; treating as failed`);
            return false;
        }
        ...
        console.warn("[auth] Turnstile siteverify threw; treating as failed:", err instanceof Error ? err.message : err);
        return false;
    }
}
...
export const authRoutes = new Elysia({ prefix: "/api/auth" })
    ...
    if (await canUseDatabase()) {
        ...
        if (user.status === "pending") {
            set.status = 403;
            return fail("Akun kamu menunggu persetujuan admin. Coba lagi setelah disetujui.");
        }
