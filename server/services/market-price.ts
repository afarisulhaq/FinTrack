...
let usdIdrCache: { value: number; expiresAt: number } | null = null;
...
async function getUsdIdrRate(): Promise<number> {
  ...
  if (usdIdrCache) return usdIdrCache.value;
...
function usdQuoteToIdr(
  quote: Omit<MarketQuote, "currency">,
  rate: number,
): MarketQuote {
  return {
    ...quote,
...
export async function getYahooFinancePrice(
  ...
  const finalResult: YahooPriceResult =
    assetClass === "crypto"
